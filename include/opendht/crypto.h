...
class RING_PUBLIC CryptoException : public std::runtime_error {
    public:
 ...
 */
class RING_PUBLIC DecryptError : public CryptoException {
    public:
...
struct RING_PUBLIC Certificate {
    Certificate() {}
 ...
 */
RING_PUBLIC Identity generateIdentity(const std::string& name, Identity ca, unsigned key_length, bool is_ca);
RING_PUBLIC Identity generateIdentity(const std::string& name = "dhtnode", Identity ca = {}, unsigned key_length = 4096);
 ...
 */
RING_PUBLIC Blob hash(const Blob& data, size_t hash_length = 512/8);
 ...
 */
RING_PUBLIC Blob stretchKey(const std::string& password, Blob& salt, size_t key_length = 512/8);

/**
 * AES-GCM encryption. Key must be 128, 192 or 256 bits long (16, 24 or 32 bytes).
 */
RING_PUBLIC Blob aesEncrypt(const Blob& data, const Blob& key);
RING_PUBLIC Blob aesEncrypt(const Blob& data, const std::string& password);

/**
 * AES-GCM decryption.
 */
RING_PUBLIC Blob aesDecrypt(const Blob& data, const Blob& key);
RING_PUBLIC Blob aesDecrypt(const Blob& data, const std::string& password);
