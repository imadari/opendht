...
RING_PUBLIC std::string print_addr(const sockaddr* sa, socklen_t slen);
RING_PUBLIC std::string print_addr(const sockaddr_storage& ss, socklen_t sslen);

struct RING_PUBLIC SockAddr : public std::pair<sockaddr_storage, socklen_t> {
public:
...
RING_PUBLIC bool operator==(const SockAddr& a, const SockAddr& b);

RING_PUBLIC std::string printAddr(const SockAddr& addr);
