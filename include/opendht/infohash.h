 ...
 */
class RING_PUBLIC InfoHash final : public std::array<uint8_t, HASH_LEN> {
public:
    ...
    RING_PUBLIC friend std::ostream& operator<< (std::ostream& s, const InfoHash& h);
    RING_PUBLIC friend std::istream& operator>> (std::istream& s, InfoHash& h);
